...
#column names for the unit and weapon dataframes
var unitCols = ['id',
                'Datasheet',
                'Name',
                'M',
...
func getHits(HitOn, attacks, abilities, debug):
    ...
    if HitOn > 0:
        ...
        #Only calculate crits if the weapon has an ability that would matter
        if (lethalHits > 0) or (SustainedHits > 0):
            for hit in range(hits):
                crits += int(rng.randi_range(1, 6) >= 6)
        #if lethal hits, we dont need to roll to wound
        #hits gets sent to the getwounds function, so
        #we subtract crits from hits and increase wounds
        #by the same amount
